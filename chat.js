  ...
  try {
    ...
    if (messages.length >= 2) {
      try {
        ...
        console.log('Lead detection verdict:', JSON.stringify(verdict), '-> leadDetected =', leadDetected);
      } catch (e) {
        console.error('Lead detection call failed:', e);
      }
    }
    ...
    if (leadDetected && process.env.RESEND_API_KEY && process.env.NOTIFY_EMAIL) {
      try {
        ...
        if (!emailRes.ok) {
          console.error('Resend email FAILED:', emailRes.status, JSON.stringify(emailData));
        } else {
          console.log('Resend email sent OK:', JSON.stringify(emailData));
        }
      } catch (e) {
        console.error('Resend email threw an error:', e);
      }
    } else if (leadDetected) {
      console.error('Lead detected but email NOT attempted — missing RESEND_API_KEY or NOTIFY_EMAIL env var.');
    }
